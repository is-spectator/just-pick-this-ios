...
- Drawer 历史会话支持搜索、置顶、重命名、删除确认和短暂撤销；置顶、隐藏和重命名会从远端事件流回流，本机刚操作但尚未回流的偏好会继续兜底。
- Drawer 置顶、重命名、删除/恢复和收藏保存、取消、恢复会同步写入 `/v1/events` 行为事件；本机状态仍即时生效，后端事件流用于跨设备恢复和效果分析。
- 关键动作使用轻触感反馈：发送、采纳、发布、滑卡、Drawer 打开/关闭、置顶、重命名、删除与恢复会话都有明确但克制的反馈。
- iOS 会尊重系统“减少动态效果”：Drawer 和来一句 Deck 会收敛缩放、旋转和弹簧位移，保留可操作性。
- 收藏页支持查看推荐卡和已采纳结果，远端收藏事件会优先回流，本机刚收藏/取消但尚未回流的状态会作为兜底保留。
- 「求一个」支持草稿、收集中、已有结果、已完成和已关闭状态；「我的求一个」会分别展示各状态数量，并支持按状态筛选记录。用户可以主动关闭当前求助，关闭后仍能回看。
- 「求一个」发布失败时保留草稿，不再假装成功；聊天页和详情页都会给出可重试的短错误提示。
- 「我的回答」支持按待采纳、已采纳和未采用筛选记录；远端回答列表优先，本机刚提交但尚未回流的记录会作为兜底保留。
...
- `PROD-009`/`STATE-002`: 当前定位失败时，地点 Sheet 会保留手动输入并提供“重试”和“去设置”两个恢复动作。
- `PROD-009`: 聊天里出现“我在/我到了 + 地点”或常见地点名时，会自动沉淀为本次决策地点并随后的请求带给后端。
- `STATE-002`: 网络层不再无条件输出 URL、响应体或解码详情，Release 运行时不会泄露调试日志。
- `CARD-007`: 聊天里的求一个卡也提供右上角 `…` 分享菜单，主卡操作仍保持「发出去」为唯一主按钮。
- `POLISH-007`/`DS-006`: 推荐卡保留内部按钮和 `…` 菜单的独立无障碍焦点，VoiceOver 不会把整张交互卡合并成单个不可操作元素。
- `CARD-003`: 推荐卡无图态不再为 `…` 菜单单独占一行，标题和理由自然上移，避免看起来像缺图留白。
- `DS-006`: 聊天求助卡的 `…` 菜单触控区域补齐到 44pt，和推荐卡操作保持同一点击规范。
- `DS-006`: Drawer 搜索清空、聊天 notice 操作、定位失败“重试 / 去设置”等按钮补齐最小触控区域，避免小图标/小胶囊按钮难点。
...
- `POLISH-006`: 推荐卡和参考图加载失败时直接退化为无图/来源文本态，不再保留空媒体框或破图式占位。
...
- `HELP-006`/`PROD-004`: 后端求助详情返回的最终 `card` 会映射为一等 `TopPick`，求助详情优先展示完整“皮皮最终推荐”，不再把最终卡降级成普通来一句文本。
- `STATE-002`/`HELP-005`: 来一句提交失败时不再本地伪成功；输入内容保留、当前 Deck 卡不切走，并用 toast 提示可重试。
- `STATE-002`/`PROD-006`: Profile、奖励、消息中心同步失败时显示“同步失败”提示，不再把后端失败伪装成空数据；未读红点失败时保持旧状态。
- `STATE-002`/`PROD-004`: 采纳推荐或来一句最终结果失败时不再本地标记完成；当前卡片保留在原位，并提示用户可重试。
- `PROD-004`/`STATE-002`: 「我的求一个」接入远端 owner 列表，远端记录优先、本地历史兜底；同步失败时提示但不清空已有记录。
